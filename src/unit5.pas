...
begin
  ...
  for i := low( P1 ) to high( P1 ) do
    begin
      ...
      if Next1 > Length( Result )
      then
        Error; // outside of string
      if Prev1 >= Next1
      then
        Error; // Next "1" is left of previous "1"
      Result[ Next1 ] := '1';
...
const
  Limit = 100; // TMemo is working very bad with huge texts
begin
  // Results.Clear;
  L := edLen.Value;
  N := edOnes.Value;
